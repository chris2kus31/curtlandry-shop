...
- **PHP JIT enabled** (`opcache.jit=1255` + `opcache.jit_buffer_size=128M`, appended to both tuning inis): **no measurable TTFB change** (0.50–0.57s fully warm, same as baseline). Expected in hindsight — Bagisto's render is thousands of short method calls, not hot loops. **Kept** (free once warm; API workloads may benefit); only cost is a longer warm-up (~15 hits) after Octane restarts.
